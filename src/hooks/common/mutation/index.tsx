...
export const useAddTask = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (body: any) => addTask(body),
    onSuccess: () => {
      queryClient.invalidateQueries([keys.GET_ALLL_TASKS]);
    },
  });
};
...
const updateTaskBucket = async (id: number, body: any) => {
  return Put({
    url: `${baseUrl}${apiUrl.UPDATE_TASKS_BUCKET}/${id}`,
    body,
  });
};

export const useUpdateTaskBucket = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, body }: { id: number; body: any }) =>
      updateTaskBucket(id, body),
    onSuccess: () => {
      queryClient.invalidateQueries([keys.GET_ALLL_BUCKETS]);
    },
  });
